...
class Dashboard(Construct):
    ...
    def __init__(self, scope: Construct, id: str, application_name: str) -> None:
        ...
        dashboard = _cloudwatch.Dashboard(self, f"{application_name}-Dashboard")
            ...
            if isinstance(element, _lamda.Function):
                dashboard.add_widgets(
                    _cloudwatch.SingleValueWidget(
                        ...
                        metrics=[
                            element.metric_duration(statistic=_cloudwatch.Stats.AVERAGE),
                            element.metric_duration(statistic=_cloudwatch.Stats.MAXIMUM)
                        ]
                    ),
                    _cloudwatch.GaugeWidget(
                        title=f"{element.function_name} Invocations",
                        metrics=[
                            element.metric_invocations(statistic=_cloudwatch.Stats.AVERAGE)
                        ]
                    )
                )
